...
\begin{enumerate}
	\item Does the tool support all required mobile operating systems? (Evaluation matrix, 7 points for each required and supported platform)
	...
	\item Does the tool support development on desktop operating systems you own? (Evaluation matrix, 7 points for each required and supported platform)
	...
	\begin{enumerate}
		\item Does the tool allow iOS builds on Windows/Linux? (Evaluation matrix, 7 points) 
		...
		\item Does the tool allow Windows builds on Mac/Linux? (Evaluation matrix, 7 points) 
	...
	\item How much are you willing to pay? What licensing do you need? What is the size of your team? (compare with the offered prices, do not assign any points, but remove tools which fail your requirements) 
	...
	\item Do you want the UI to look the same on all operating systems, or use the native look and feel? Does the tool support it? (do not assign any points, but remove tools which fail your requirements) 
	...
	\begin{enumerate}
		\item If you chose native look, do you want to code a single UI layer for all operating systems, or a custom layer for each mobile OS? Does the tool support it? (do not assign any points, but remove tools which fail your requirements) 
	...
	\item Does your project require multithreading and does the development tool support it? (Evaluation matrix, 7 points) 
	...
	\item Do you want to access the media, use the camera and microphone, play audio and video? Does the tool provide the API? (Evaluation matrix, 7 points for each required and supported feature) 
	...
	\item Do you want to use geolocation, accelerometer or gyroscope? Does the tool provide the API? (Evaluation matrix, 7 points for each required and supported feature) 
	...
	\item Do you want to access the device state (battery status, network availability, OS version, etc.)? Does the tool provide necessary APIs? (Evaluation matrix, 7 points) 
	...
	\item Do you want to use a background process? Does the tool support them? (Evaluation matrix, 7 points) 
	...
	\item Do you want to use PUSH notifications? Does the tool support them? (Evaluation matrix, 7 points) 
	...
	\item Do you need to create custom plugins or invoke native libraries? Does the tool support it? (Evaluation matrix, 7 points) 
	...
	\item Is app monitoring and crash analytics required by the project? Are they supported by the tool? (Evaluation matrix, 7 points) 
...
\begin{enumerate}
	\item How would you characterize the complexity of your app? (TBD)
	TBD
	
	\item Is designer or previewer tool available? (Evaluation matrix, 5 points) 
	...
	\item Do you want to have different UI for phone and tablet? Does the tool support multiple or responsive layouts? (Evaluation matrix, 5 points) 
	...
	\item Does the design include any platform-specific overrides? Does the tool support them? (Evaluation matrix, 5 points) 
	...
	\item Will the application work with the file system? Is the tool able to access and manage it? (Evaluation matrix, 5 points) 
	...
	\item Will the application require persistent storage? Can the tool work with app properties or mobile database? (Evaluation matrix, 5 points for each required and supported database) 
	...
	\item Do you want to use Bluetooth, NFC, fingerprint or barcode scanner? Is their API available in given tool? (Evaluation matrix, 5 points for each required and supported feature) 
	...
	\item Is integration with assistant service (Alexa, Cortana, Google Assistant, Siri) possible? (Evaluation matrix, 5 points for each required and supported assistant) 
	...
	\item Does the tool support debugging? (Evaluation matrix, 5 points) 
	...
	\item Does the tool support unit testing? (Evaluation matrix, 5 points) 
	...
	\item Does the tool support automated UI testing? (Evaluation matrix, 5 points) 
	...
	\item Does the tool support app profiling? (Evaluation matrix, 5 points) 
...
\begin{enumerate}
	\item Does the tool allow development in a programming language you already have experience with? (Evaluation matrix, 3 points for each required and supported language) 
	...
	\item Are MBaaS features or cloud connection necessary? Are they available? (Evaluation matrix, 3 points) 
	...
	\item Do you need an embedded web browser? Does the tool allow its use? (Evaluation matrix, 3 points) 
	...
	\item Do you want to access the address book and be able to make a call, or send SMS? Does the tool provide all necessary APIs? (Evaluation matrix, 3 points) 
	...
	\item Is a testing environment for multiple devices required? Does the tool feature any? (Evaluation matrix, 3 points) 
	...
	\item Does the tool allow continuous integration, if necessary? (Evaluation matrix, 3 points) 
	...
	\item Are closed test groups required and supported? (Evaluation matrix, 3 points) 
	...
	\item Is OpenGL or WebGL required? Is it supported? (Evaluation matrix, 3 points) 
	...
	\item Does the tool support creating 2D games, if necessary? (Evaluation matrix, 3 points) 
	...
	\item Does the tool support augmented or virtual reality, if necessary? (Evaluation matrix, 3 points) 
...
Do the developers have experience with a multi-platform development tool?
...
What IDEs do they use?
Although this may not be as critical, the familiarity with an IDE or other productivity-boosting software may increase the development speed. Also, reusing an already licensed software will cut the costs.
Do you have specialists for individual mobile operating systems?
...
What is the performance of the resulting apps?
What is the size of the resulting apps?
What is the learning curve?
How long does it take to create a simple app? How long does it take to master the tool?
How often is the tool updated? How long does it take to react to a new OS release?
How comprehensive is the documentation?
How large and how busy is the community?
How buggy is the tool? How are bugs handled?
What are some prominent apps developed using the tool?
